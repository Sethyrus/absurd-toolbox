...
class _SoundRecorderScreenState extends State<SoundRecorderScreen> {
  ...
  String _mFileName = 'temp_recording.mp4';
  String _mPath = '';
  ...
  void _listofFiles() async {
    ...
    setState(() {
      _file = io.Directory(path).listSync();
  ...
  Future<void> initRecorder() async {
    ...
    if (!await _mRecorder.isEncoderSupported(_codec)) {
      ...
      _mFileName = 'temp_recording.webm';
  ...
  void startRecording() async {
    ...
    await _mRecorder.startRecorder(
        toFile: _mPath + '/' + _mFileName, codec: _codec);
  }
  ...
  void stopRecording() async {
    ...
    // await _mPlayer.startPlayer(fromURI: path, codec: Codec.aacADTS);
  }
  ...
  _Fn? getPlaybackFn() {
    if (!_playerInit || !_mPlaybackReady || !_mRecorder.isStopped) {
      return null;
    }
  ...
  Widget build(BuildContext context) {
    return Layout(
      ...
      content: Container(
        child: Column(
          children: [
            Text('_recorderInit ' + _recorderInit.toString()),
            Text('_playerInit ' + _playerInit.toString()),
            // Flexible(child: SimpleRecorder()),
            Row(children: [
              ElevatedButton(
